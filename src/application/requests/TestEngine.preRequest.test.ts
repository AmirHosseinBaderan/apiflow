...
describe('TestEngine.preRequest', () => {
  ...
  it('passes when script returns truthy', () => {
    const step: TestStep = {
      id: 'p1',
      name: 'check',
      kind: { type: 'script', source: 'pm.variables.set("token", "abc"); return true;' },
      expression: '',
    };
  ...
  it('sets runtime variable via pm.variables.set', () => {
    const step: TestStep = {
      id: 'p2',
      name: 'set token',
      kind: { type: 'script', source: 'pm.variables.set("token", "xyz"); return true;' },
      expression: '',
    };
    const c = ctx();
  ...
  it('errors on script exception', () => {
    const step: TestStep = {
      id: 'p3',
      name: 'throws',
      kind: { type: 'script', source: 'throw new Error("nope")' },
      expression: '',
    };
