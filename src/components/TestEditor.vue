<template>
  <div>
    <v-row v-for="t in local" :key="t.id" dense class="mb-1" align="center">
      <v-col cols="3"><v-text-field v-model="t.name" placeholder="name" density="compact" hide-details /></v-col>
      <v-col cols="3">
        <v-select
          v-model="t.kind.type"
          :items="kinds"
...
function add() {
  const step: TestStep = { id: createId('tst'), name: 'Status is 200', kind: { type: 'statusEquals', value: 200 }, expression: '200' };
...
function onKindChange(id: string, type: string) {
  const next = props.modelValue.map((t) => {
    ...
    let kind: TestKind;
    switch (type) {
      case 'statusEquals': kind = { type: 'statusEquals', value: 200 }; break;
      case 'bodyExists': kind = { type: 'bodyExists', path: '' }; break;
      case 'durationLessThan': kind = { type: 'durationLessThan', valueMs: 1000 }; break;
      case 'script': kind = { type: 'script', source: 'return response.status === 200' }; break;
      default: kind = { type: 'statusEquals', value: 200 };
    }
    return { ...t, kind };
  });
