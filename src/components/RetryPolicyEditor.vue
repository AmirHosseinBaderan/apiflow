<template>
  <div class="pa-2">
    <v-switch v-model="local.enabled" label="Enable retry" hide-details />
    <v-row dense>
      <v-col cols="6"><v-text-field v-model.number="local.maxAttempts" label="Max attempts" type="number" hide-details /></v-col>
      <v-col cols="6"><v-text-field v-model.number="local.initialDelayMs" label="Initial delay (ms)" type="number" hide-details /></v-col>
    </v-row>
    <v-select v-model="local.backoff" :items="['fixed','exponential']" label="Backoff" hide-details />
    <v-combobox v-model="local.retryOn" :items="['networkError','timeout','status5xx','statusCode']" label="Retry on" multiple chips hide-details />
  </div>
</template>
