      ...
      <v-col cols="12" v-if="node?.kind === 'collection'">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-btn class="mt-3" color="primary" :disabled="workflowRequestIds.length < 2" @click="runWorkflow">
              Run workflow
            </v-btn>
...
function openNode(item: CollectionTreeNode) {
  ...
  if (!cid) return;
  if (item.kind === 'folder') router.push({ name: 'node', params: { collectionId: cid, folderId: item.id } });
  else if (item.kind === 'request') router.push({ name: 'request', params: { collectionId: cid, requestId: item.id } });
}
...
function buildCondition() {
  if (conditionType.value === 'statusEquals') return { type: 'statusEquals' as const, value: Number(conditionValue.value) || 200 };
